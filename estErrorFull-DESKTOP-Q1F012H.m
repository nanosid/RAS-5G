	...
	for i1 = 1:length(sigma)
		for i2 = 1:length(numants)
			...
			for var = 1:N
				...
				cvx_begin
					variable w(1,M) complex
					minimize(norm(hest_gU_err*w' - ones(K,1)));
					subject to
						norm(hest_gR_temp*w') <= e;
				cvx_end
				w_opt_err_k = w;
				...
				for i = 1:K
					...
					% Opt error in h_k
					UE_wf_bf = (wf*w_opt_err_k)*hest_gU_all(i,:)';
					...
					% Opt error in h_gR
					UE_wf_bf = (wf*w_opt_err_gR)*hest_gU_all(i,:)';
...
xticklabels(["1","10^{-1}","10^{-2}","10^{-3}","10^{-4}","10^{-5}"]);
ylabel('Change in Int. Pow. (dBW)');
ylim([-50 350]);
leg = legend("No Null, M=8","w_{opt}, M=8","w_{heu}, M=8","No Null, M=16","w_{opt}, M=16","w_{heu}, M=16",'location','northeast');
...
plot(mu,permute(mean(RAS_null_heu(1,:,:,1,:) - RAS_null_heu_err_k(1,:,:,1,:),5),[3 2 1 4]))
hold on
plot(mu,permute(mean(RAS_null_opt(1,:,:,1,:) - RAS_null_opt_err_k(1,:,:,1,:),5),[3 2 1 4]))
semiPlots
f.Position = [6 6 3 3];
leg = legend("w_{heu}, \sigma^2=10^{-5}","w_{heu}, \sigma^2=5\times10^{-5}","w_{heu}, \sigma^2=10^{-4}",...
	"w_{opt}, \sigma^2=10^{-5}","w_{opt}, \sigma^2=5\times10^{-5}","w_{opt}, \sigma^2=10^{-4}");
...
plot(mu,permute(mean(sumrate_heu(1,:,:,1,:) - sumrate_heu_err(1,:,:,1,:),5),[3 2 1 4]))
hold on
plot(mu,permute(mean(sumrate_opt(1,:,:,1,:) - sumrate_def_err(1,:,:,1,:),5),[3 2 1 4]))
semiPlots
f.Position = [6 6 3 3];
leg = legend("w_{heu}, \sigma^2=10^{-5}","w_{heu}, \sigma^2=5\times10^{-5}","w_{heu}, \sigma^2=10^{-4}",...
	"w_{opt}, \sigma^2=10^{-5}","w_{opt}, \sigma^2=5\times10^{-5}","w_{opt}, \sigma^2=10^{-4}");
xlabel("\mu");
ylabel("\Delta SR_{hgR}");
leg.ItemTokenSize = [10 18];
leg.FontSize = 7;
title("M=8, f_c=5.9, K=16");

plot(mu,permute(mean(sumrate_heu(1,:,:,1,:) - sumrate_heu_err_k(1,:,:,1,:),5),[3 2 1 4]))
hold on
plot(mu,permute(mean(sumrate_opt(1,:,:,1,:) - sumrate_def_err_k(1,:,:,1,:),5),[3 2 1 4]))
semiPlots
f.Position = [6 6 3 3];
leg = legend("w_{heu}, \sigma^2=10^{-5}","w_{heu}, \sigma^2=5\times10^{-5}","w_{heu}, \sigma^2=10^{-4}",...
	"w_{opt}, \sigma^2=10^{-5}","w_{opt}, \sigma^2=5\times10^{-5}","w_{opt}, \sigma^2=10^{-4}");
