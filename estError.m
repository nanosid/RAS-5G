...
UE_snr_err_k = zeros(3,length(mu),length(sigma),length(numants),N,numUEs(end));
UE_snr_err_gR = zeros(3,length(mu),length(sigma),length(numants),N,numUEs(end));
UE_snr_bf = zeros(size(UE_snr_err_k));
RAS_null_err_k = zeros(3,length(mu),length(sigma),length(numants),N);
RAS_null_err_gR = zeros(3,length(mu),length(sigma),length(numants),N);
RAS_null_bf = zeros(size(RAS_null_err_k));
sumrate_err_k = zeros(size(RAS_null_err_k));
sumrate_err_gR = zeros(size(RAS_null_err_k));
sumrate_bf = zeros(size(RAS_null_err_k));
		...
		for i3 = 1:length(numants)
			...
			parfor var = 1:N
				...
				for i=1:K
					...
					hest_gU_err(i,:) = hest_gU_all(i,:) + mu(i1) + sigma(i2)*complex(randn(size(hest_gR_temp)),randn(size(hest_gR_temp)));
				...
				for i = 1:K
					...
					UE_snr_err_k(rowidx,i1,i2,i3,var,i) = snr(UE_wf_bf,noise_UE(:,i));
					if UE_snr_err_k(rowidx,i1,i2,i3,var,i) > -1
						sumrate_err_k(rowidx,i1,i2,i3,var) = sumrate_err_k(rowidx,i1,i2,i3,var) + log2(1+UE_snr_err_k(rowidx,i1,i2,i3,var,i));
					else
						sumrate_err_k(rowidx,i1,i2,i3,var) = 0;
						break;
					end

					% Error in h_gR
					UE_wf_bf = (wf*w_copt_err_gR.')*hest_gU_all(i,:)';
					UE_snr_err_gR(rowidx,i1,i2,i3,var,i) = snr(UE_wf_bf,noise_UE(:,i));
					if UE_snr_err_gR(rowidx,i1,i2,i3,var,i) > -1
						sumrate_err_gR(rowidx,i1,i2,i3,var) = sumrate_err_gR(rowidx,i1,i2,i3,var) + log2(1+UE_snr_err_gR(rowidx,i1,i2,i3,var,i));
					else
						sumrate_err_gR(rowidx,i1,i2,i3,var) = 0;
						break;
					end
					
					% No error
					UE_wf_bf = (wf*w_copt.')*hest_gU_all(i,:)';
					UE_snr_bf(rowidx,i1,i2,i3,var,i) = snr(UE_wf_bf,noise_UE(:,i));
					if UE_snr_bf(rowidx,i1,i2,i3,var,i) > -1
						sumrate_bf(rowidx,i1,i2,i3,var) = sumrate_bf(rowidx,i1,i2,i3,var) + log2(1+UE_snr_bf(rowidx,i1,i2,i3,var,i));
					else
						sumrate_bf(rowidx,i1,i2,i3,var) = 0;
						break;
					end
				end
				RAS_null_err_k(rowidx,i1,i2,i3,var) = 10*log10(norm(hest_gR_temp*w_copt_err_k).^2);
				RAS_null_err_gR(rowidx,i1,i2,i3,var) = 10*log10(norm(hest_gR_temp*w_copt_err_gR).^2);
				RAS_null_bf(rowidx,i1,i2,i3,var) = 10*log10(norm(hest_gR_temp*w_copt).^2);
			end
		end
	end
	end
